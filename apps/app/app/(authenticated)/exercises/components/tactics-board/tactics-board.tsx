...
export function TacticsBoard({ initialData, onSave }: TacticsBoardProps = {}) {
  ...
  useEffect(() => {
    const timeout = window.setTimeout(() => setShowHelpHint(false), 5200);
  ...
  const zoomAtPoint = useCallback((screenPoint: BoardPoint, nextScale: number) => {
    setViewport((current) => {
      const boardPoint = {
  ...
  const finishInteraction = useCallback(() => {
    ...
    if (selectionBox) {
      ...
      const selectedIds = frame.elements
        .filter((element) => boundsIntersect(getElementBounds(element), selectionBounds))
    ...
    if (
      ...
      addElement(
        createArrowElement(draft.kind === "pass-arrow" ? "pass" : "run", draft.points)
      );
    }
  ...
  return (
    <section className="relative isolate overflow-hidden rounded-2xl border border-border-primary bg-bg-primary">
      <div
        className="relative h-[calc(100dvh-7rem)] min-h-[560px] touch-none bg-bg-quaternary md:h-[calc(100dvh-8rem)]"
        ref={containerRef}
      >
        <Stage
          height={size.height}
        ...
        >
          <Layer>
            <Group scaleX={viewport.scale} scaleY={viewport.scale} x={viewport.x} y={viewport.y}>
          ...
          <Layer>
            <Group scaleX={viewport.scale} scaleY={viewport.scale} x={viewport.x} y={viewport.y}>
              ...
              {selectionBox ? <SelectionBoxNode selectionBox={selectionBox} /> : null}
            </Group>
          </Layer>
        </Stage>
        ...
        {onSave ? (
          <div className="absolute top-4 right-4 z-40">
            <button
              className="rounded-full bg-brand px-4 py-2 font-medium text-brand-foreground text-sm shadow-floating transition-transform hover:scale-105"
              onClick={handleSave}
              type="button"
            >
              Guardar pizarra
        ...
        {touchDrag ? (
          <div className="pointer-events-none absolute top-4 right-4 z-30 rounded-2xl border border-border-primary bg-bg-primary/90 p-3 shadow-floating backdrop-blur">
            <p className="font-medium text-text-secondary text-xs">Precision touch</p>
            <p className="font-semibold text-sm text-text-primary">
              x {Math.round(touchDrag.point.x)} · y {Math.round(touchDrag.point.y)}
            </p>
          </div>
        ) : null}
      </div>
    ...
    <Group listening={false}>
      ...
      {showFull || showHalf ? (
        <>
          ...
          <Circle
            ...
            y={FIELD_HEIGHT / 2}
          />
          <Circle fill="rgba(255,255,255,0.86)" radius={7} x={FIELD_WIDTH / 2} y={FIELD_HEIGHT / 2} />
        </>
      ) : null}
      ...
      {showPenalty ? (
        <Group>
          ...
          />
          <Circle fill="rgba(255,255,255,0.86)" radius={7} x={FIELD_WIDTH - 332} y={FIELD_HEIGHT / 2} />
        </Group>
      ) : null}
    </Group>
  );
...
function PenaltyArea({ side }: { readonly side: "left" | "right" }) {
  ...
  return (
    <Group>
      ...
      />
      <Circle fill="rgba(255,255,255,0.86)" radius={6} x={spotX} y={FIELD_HEIGHT / 2} />
    </Group>
  );
}
...
function BoardElementNode({
  ...
  readonly onDragEnd: (id: string, event: Konva.KonvaEventObject<DragEvent>) => void;
  readonly onDragMove: (id: string, event: Konva.KonvaEventObject<DragEvent>) => void;
  ...
  return (
    ...
    >
      {element.type === "player" ? (
        <PlayerNode element={element} isSelected={isSelected} teamColors={teamColors} />
      ) : null}
...
function BallNode({ isSelected }: { readonly isSelected: boolean }) {
  return (
    <Group>
      {isSelected ? <Circle radius={20} stroke="#f8fafc" strokeWidth={3} /> : null}
...
function EquipmentNode({
  ...
  if (element.kind === "cone") {
    return (
      <Group>
        {isSelected ? <Circle radius={22} stroke="#f8fafc" strokeWidth={3} /> : null}
        <RegularPolygon
          fill="#f97316"
          radius={18}
          rotation={180}
  ...
  if (element.kind === "pole") {
    return (
      <Group>
        {isSelected ? <Circle radius={20} stroke="#f8fafc" strokeWidth={3} /> : null}
        <Rect fill="#f59e0b" height={44} offsetX={3} offsetY={22} radius={3} width={6} />
      </Group>
    );
  }
  ...
  return (
    <Group>
      {isSelected ? <Rect height={42} offsetX={30} offsetY={21} stroke="#f8fafc" strokeWidth={3} width={60} /> : null}
      <Rect
        height={34}
        offsetX={26}
        offsetY={17}
