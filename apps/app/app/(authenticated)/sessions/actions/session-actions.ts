...
export async function createSession(
  ...
  try {
    ...
    let recurrenceParsed: { byday: Weekday[]; until: string } | undefined;
    if (typeof recurrenceRaw === "string" && recurrenceRaw.trim().length > 0) {
      try {
        const parsed = JSON.parse(recurrenceRaw) as {
          byday?: string[];
          until?: string;
        };
        if (parsed.byday && parsed.until) {
          recurrenceParsed = {
            byday: parsed.byday as Weekday[],
            until: parsed.until,
          };
        }
      } catch {
