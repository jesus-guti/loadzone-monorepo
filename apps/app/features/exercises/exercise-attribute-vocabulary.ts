/**
 * Valores canónicos de los enums de `Exercise` (alineados con Prisma) y etiquetas
 * para UI staff. El servidor valida con los mismos valores vía `exercise-form-schema`.
 */

export function enumValuesFromOptions<
  const T extends readonly { readonly value: string }[],
>(options: T): [T[number]["value"], ...T[number]["value"][]] {
  if (options.length === 0) {
    throw new Error("enumValuesFromOptions: options must not be empty");
  }
  const values = options.map((o) => o.value) as T[number]["value"][];
...
export type ExerciseComplexityValue =
  (typeof COMPLEXITY_OPTIONS)[number]["value"];

export function staffLabelForExerciseComplexity(
  value: ExerciseComplexityValue
): string {
  const row = COMPLEXITY_OPTIONS.find((o) => o.value === value);
