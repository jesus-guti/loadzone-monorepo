...
vi.mock("@repo/database/create-injury", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@repo/database/create-injury")>();
  return {
    ...actual,
    createStaffInjury: stubs.createStaffInjury,
  };
});
...
describe("createInjury / promotePainAlert Care Alert contract (JES-54)", () => {
  ...
  it("createInjury creates Injury and never calls evaluateAndEmitCareAlert", async () => {
    ...
    stubs.createStaffInjury.mockResolvedValue({
      id: "inj_1",
