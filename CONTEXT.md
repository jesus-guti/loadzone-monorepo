...
**Streak Cromo**:
Player-facing identity card in `apps/player` that visually evolves with the **Recoverable Streak**. Shows staff-uploaded **Player** photo (calm silhouette when missing) and **Club** crest from `Club.logoUrl` only (omit when null; never **Team** logo). Optional **Playing Position** line when set. Portrait overprint shows the optional shirt (**dorsal**) as a hollow numeral (rank-stamp lab variant C treatment) when `Player.shirtNumber` is set. Footer ink seal (lab variant A) shows **Team streak rank** (`#1` = longest Recoverable Streak among non-archived teammates; omit when this Player’s streak is 0). Footer bottom-left shows **Team** name in cromo serif. Recoverable Streak count stays in the header racha pill and LOADZONE pill — not duplicated as a fire disc on the card. Material tier names (Bronce, Oro, Esmeralda, …) are not printed on the card; vivid per-tier chrome is player-local CSS — not a second staff / design-system brand hue. Foil recipes (never printed on the card, not Pokémon rarities): **plate** on Bronce and Plata (distinct plate pattern per those two tiers); **holo** on Oro, Platino, and Esmeralda (distinct holo pattern per those three); **spark** on Diamante only. The cromo reads as a physical card: rectangular outer **frame**, portrait in an arched window inside it. Foil on tilt lives on metallic chrome — outer frame, LOADZONE pill, and shirt overprint — not on the Team streak-rank seal (ink) and not as a holographic wash over the photo (portrait stays matte). A pointer **sleeve glare** (white beam / hotspot) may cross the whole card, including the portrait: that is specular light, not a foil recipe. Player name and **Team** name stay ink. Full cromo (Racha sheet overlay and today’s completion) uses modest 3D tilt plus pointer/touch foil; an ambient loop yields while tracking, then returns. Device tilt is opt-in from the Racha sheet and may continue on completion if already granted. The compact header pill is not a cromo and is not tracked. `prefers-reduced-motion`: static recipe paint only — no loop, pointer tracking, or gyro. When **Recoverable Streak** crosses a **CromoTier** threshold, today’s completion shows a short morph from the previous foil recipe to the new one (no material names). Opening Racha in that same completion visit may replay the morph once. `prefers-reduced-motion`: the new recipe with no morph. Football-identity teaser only — never claims real performance / health scoring. Distinct from staff Wellness Tarjetas / admin cromos. Empty **Playing Position** omits the line (no «Sin posición» placeholder).
