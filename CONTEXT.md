...
**Streak Cromo**:
Player-facing identity card in `apps/player` that visually evolves with the **Recoverable Streak**. Football-identity teaser only — never claims real performance / health scoring. Distinct from staff Wellness Tarjetas / admin cromos. May show an optional **Playing Position** line (POR / DEF / MED / DEL) when set on the **Player**; empty omits the line (no «Sin posición» placeholder).
